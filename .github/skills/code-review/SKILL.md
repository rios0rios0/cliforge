---
name: code-review
description: "Review pull requests and diffs in cliforge — the shared Go library providing self-update and platform abstraction for CLI tools — against the rios0rios0/guide standards, with extra weight on public-API stability, cross-platform behaviour, and update-download safety. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `cliforge`

`cliforge` is a **library**, not a binary: `pkg/platform` abstracts the OS (download, extract, move, remove, chmod) and `pkg/selfupdate` implements the GitHub-Releases self-update flow used by `autobump`, `autoupdate`, `terra`, `ccswitch`, and others. Every exported symbol is a contract with those consumers.
...
- The absence of a binary or CLI — that is the point of the module.
